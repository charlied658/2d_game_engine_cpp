...
namespace Highlight {
     ...
     */
    void init() {
        highlighted_objects = new SpriteManager::sprite_manager *[1000];
    ...
    static void reset() {
        ...
        for (int i = 0; i < game_object_count; i++) {
            SpriteManager::set_highlighted(&game_objects[i], false);
            SpriteManager::set_dragging(&game_objects[i], false);
        }
    ...
    static void highlight_objects() {
        ...
        if (SelectionBox::is_multiselect()) {
            ...
            SpriteRenderer::highlight_sprites(highlighted_objects, &highlighted_object_count, selection_box);
        } else {
            // Highlight the object that the mouse cursor is over
            if (!ImGuiLayer::want_mouse_capture() && !Drag::is_dragging_objects()) {
                SpriteRenderer::highlight_sprite(&highlighted_obj);
            }
            ...
            if (highlighted_obj && highlighted_obj->selected) {
                ...
                for (int i = 0; i < selected_object_count; i++) {
                    SpriteManager::set_highlighted(selected_objects[i], true);
                }
            }
        }
    }
