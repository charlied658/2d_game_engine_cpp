...
#include "editor/render/line.h"
#include "editor/render/line_renderer.h"

namespace GridLines {

    static Editor::Line::line *line_list_vertical;
    static Editor::Line::line *line_list_horizontal;
     ...
     */
    void init() {
        line_list_vertical = new Editor::Line::line[1000];
        line_list_horizontal = new Editor::Line::line[1000];
    ...
    void update() {
        ...
        index = 0;
        while (lineX < endX) {
            Editor::Line::init(&line_list_vertical[index], glm::vec2(lineX, startY), glm::vec2(lineX, endY), line_width, color);
            if (index >= line_count_vertical) {
                if (line_count_vertical < 1000) {
                    line_count_vertical++;
                    Editor::LineRenderer::add_line(&line_list_vertical[index]);
                } else {
                    break;
                }
            }
        ...
        index = 0;
        while (lineY < endY) {
            Editor::Line::init(&line_list_horizontal[index], glm::vec2(startX, lineY), glm::vec2(endX, lineY), line_width, color);
            if (index >= line_count_horizontal) {
                if (line_count_horizontal < 1000) {
                    line_count_horizontal++;
                    Editor::LineRenderer::add_line(&line_list_horizontal[index]);
                } else {
                    break;
                }
            }
