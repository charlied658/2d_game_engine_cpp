...
namespace Selected {
     ...
     */
    void init() {
        selected_objects = new Editor::SpriteManager::sprite_manager *[1000];
    ...
    void reset_selected() {
        if (selected_object_count > 0) {
            for (int i = 0; i < selected_object_count; i++) {
                Editor::SpriteManager::set_selected(selected_objects[i], false);
            }
            selected_object_count = 0;
        }
    }
    ...
    void select_object() {
        ...
        Editor::SpriteManager::set_selected(highlighted_obj, true);
    ...
    void unselect_object() {
        ...
        for (int i = 0; i < selected_object_count; i++) {
            if (selected_objects[i] == highlighted_obj) {
                Editor::SpriteManager::set_selected(highlighted_obj, false);
                found_obj = true;
            }
    ...
    void select_highlighted_object() {
        if (!Drag::is_dragging_objects()) {
            ...
            Editor::SpriteManager::set_selected(selected_objects[0], true);
    ...
    void select_highlighted_objects() {
        if (highlighted_object_count > 0) {
            for (int i = 0; i < highlighted_object_count; i++) {
                ...
                Editor::SpriteManager::set_selected(selected_objects[i], true);
            }
