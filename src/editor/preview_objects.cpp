...
namespace Preview {
    ...
    static SpriteManager::sprite_manager *preview_objects;
    static int preview_object_count;
    static SpriteManager::sprite_manager holding_object_preview;
     ...
     */
    void init() {
        preview_objects = new SpriteManager::sprite_manager[1000];
    ...
    void render() {
        ...
        for (int i = 0; i < selected_object_count; i++) {
            ...
            if (i >= preview_object_count && preview_object_count < 1000) {
                SpriteRenderer::add_sprite(&preview_objects[i]);
        ...
        if (preview_object_count > selected_object_count) {
            for (int i = selected_object_count; i < preview_object_count; i++) {
                SpriteManager::set_visible(&preview_objects[i], false);
            }
        }
    }
    ...
    void render_holding() {
        ...
        if (holding_object && Holding::is_holding()) {
            ...
            if (!holding_preview_added) {
                ...
                SpriteRenderer::add_sprite(&holding_object_preview);
            }
        } else {
            SpriteManager::set_visible(&holding_object_preview, false);
        }
    }
    ...
    void generate_preview_object(SpriteManager::sprite_manager *obj, glm::vec2 position, glm::vec2 scale, Sprite::sprite *sprite) {
        SpriteManager::init(obj, "preview", position, scale, -5, sprite);
        SpriteManager::set_color(obj, glm::vec4(1.0f, 1.0f, 1.0f, 0.0f));
        SpriteManager::set_saturation(obj, 0.3f);
        obj->pickable = false;
    }
}
