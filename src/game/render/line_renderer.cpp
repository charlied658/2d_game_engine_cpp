...
namespace Game {
    namespace LineRenderer {
        ...
        static Game::LineBatch::line_batch batches[100];
        ...
        void render() {
            ...
            for (int i = 0; i < batch_count; i++) {
                Game::LineBatch::render(&batches[i]);
            }
        }
         ...
         */
        void add_line(Game::Line::line *line) {
            for (int i = 0; i < batch_count; i++) {
                // If there is enough space to add the object, add it to the first available batch
                if (batches[i].has_room) {
                    Game::LineBatch::add_line(&batches[i], line);
                    return;
                }
            }
            // If no batch has space for the line, create a new batch
            Game::LineBatch::line_batch batch{};
            Game::LineBatch::init(&batch, max_batch_size);
            batches[batch_count] = batch;
            Game::LineBatch::add_line(&batches[batch_count], line);
            batch_count++;
        }
    }
}
